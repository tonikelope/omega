...
def help_settings_nei(item):
    xbmcgui.Dialog().textviewer("AYUDA", "[B]¿Qué valor se recomienda poner en [COLOR yellow]hilos[/COLOR] de descarga DEBRID/MEGA?[/B]\nPor norma general, este valor debe dejarse a 1 a no ser que por algún motivo el vídeo que estemos reproduciendo vaya más lento de lo habitual, nuestro ISP nos limite de alguna manera o se trate de un vídeo de bitrate muy alto (por ejemplo 4K REMUX). En ese caso, podemos empezar con 4 hilos e ir subiendo poco a poco.\n\n[B]¿Qué se debe poner en [COLOR yellow]tamaño de buffer DEBRID/MEGA[/COLOR]?[/B]\nEn caso de utilizar más de un hilo de descarga, se recomienda dejar este buffer en el valor por defecto (50MB para MEGA y 100MB para DEBRID) o como mucho aumentarlo hasta el doble (si sólo se usa un hilo de descarga, este valor se ignorará).\n\n[B]¿Qué valor se debe usar para el [COLOR yellow]buffer de KODI[/COLOR]?[/B]\nEste ajuste es importante. Si la reproducción funciona sin saltos, no hace falta tocarlo. En caso contrario o para soportar vídeos de bitrate muy elevado se recomienda subirlo hasta 150/200 MB (ojo con subirlo mucho porque podemos quedarnos sin memoria RAM en el dispositivo).\n\n[B]¿Y la [COLOR yellow]velocidad[/COLOR] de lectura del buffer de KODI?[/B]\nSe recomienda nuevamente dejarlo por defecto o 4x y aumentarlo hasta 8x para vídeos con bitrate muy alto.\n\n[B]NOTA:[/B] los hilos ayudan a exprimir la velocidad de descarga al máximo y los buffers a AMORTIGUAR bajadas puntuales en la velocidad de descarga, pero la limitación real es el ancho de banda de tu conexión a internet y el bitrate del vídeo en cuestión. (Por ejemplo, para reproducir por streaming de forma fluida una película 4K REMUX (unos 80GB) se requiere como mínimo una conexión de 500Mbps, siendo recomendable 1Gbps).")
