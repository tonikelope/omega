...
def checkHDR(title):
    return re.search(r"[^a-z](hdr|dv|dolby\-?vision)[^a-z]", title, re.IGNORECASE | re.DOTALL)


def checkREMUX(title):
    return re.search(r"[^a-z](remux|4k\-?remux|uhd\-?remux)[^a-z]", title, re.IGNORECASE | re.DOTALL)
