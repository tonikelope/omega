...
def help_settings_nei(item):
    xbmcgui.Dialog().textviewer("AYUDA", "[B]¿Qué valor se recomienda poner en [COLOR yellow]hilos[/COLOR] de descarga DEBRID/MEGA?[/B]\nPor norma general, este valor debe dejarse a 1 a no ser que por algún motivo el vídeo que estemos reproduciendo vaya más lento de lo habitual, nuestro ISP nos limite de alguna manera o se trate de un vídeo de bitrate muy elevado (por ejemplo 4K REMUX). En ese caso, podemos empezar con 2 hilos e ir subiendo poco a poco.\n\n[B]¿Qué se debe poner en [COLOR yellow]tamaño de buffer DEBRID/MEGA[/COLOR]?[/B]\nEn caso de utilizar más de un hilo de descarga, se recomienda dejar este buffer en 50MB (si sólo se usa un hilo de descarga, este valor se ignorará).\n\n[B]¿Qué valor se debe usar para el [COLOR yellow]buffer de KODI[/COLOR]?[/B]\nEste ajuste es importante. Si la reproducción funciona sin saltos, no hace falta tocarlo. En caso contrario o para soportar vídeos de bitrate muy elevado como 4K REMUX se recomienda irlo subiendo hasta 256MB (ojo con subirlo mucho porque podemos quedarnos sin memoria RAM en el dispositivo).\n\n[B]¿Y la [COLOR yellow]velocidad[/COLOR] de carga del buffer de KODI?[/B]\nSe recomienda dejarlo a 6x e irlo aumentando hasta 12x si pensamos reproducir vídeos de bitrate muy elevado como 4K REMUX\n\n[I][B]Nota1:[/B] los hilos ayudan a exprimir la velocidad de descarga al máximo y los buffers a suavizar bajadas puntuales en la velocidad de descarga, pero la limitación real es el ancho de banda de tu conexión a Internet y el bitrate del vídeo en cuestión. (Por ejemplo, para reproducir por streaming de forma fluida una película de 2 horas 4K REMUX (80 GB aprox) se requiere como mínimo una conexión REAL de 200Mbps (ojo al WIFI), siendo recomendable 300Mbps o más).[/I]\n\n[B]IMPORTANTE:[/B] a partir de la [B][COLOR yellow]versión 21[/COLOR][/B] de KODI, el tamaño del buffer y su velocidad de carga se configuran desde los [B]ajustes generales de KODI[/B] en modo avanzado/experto en la sección SERVICIOS -> ALMACENAMIENTO EN CACHÉ (por tanto, lo que se ponga en estos dos valores en ajustes de OMEGA será ignorado por KODI).\n\n[B]Al entrar a OMEGA han desaparecido casi todos los iconos[/B]\nA veces tras una actualización de ALFA, OMEGA puede des-sincronizarse. El 99% de las veces se soluciona pulsando en [B]FORZAR RE-LOGIN[/B], cerrando KODI y volviendo a entrar.")
...
def bibliotaku_series_temporadas(item):
    ...
    ajustes_item.title = "[COLOR yellow][B]AJUSTES[/B][/COLOR]"
...
def bibliotaku_pelis_megacrypter(item):
    ...
    recomendados_item.tmdbType = 'movie'
...
def foro(item, episode_count_call=False):
    ...
    if '<h3 class="catbg">Subforos</h3>' in data:
        ...
        recomendados_item.tmdbType = 'tv' if item.contentSerieName != "" else 'movie'
    ...
    if isinstance(resultados, dict) and 'results' in resultados:
        for peli in resultados['results']:
            itemlist.append(Item(
                ...
                viewmode='list',
                viewcontent = 'movies',
                contentTitle = peli['title' if item.tmdbType == 'movie' else 'name'],
                contentSerieName = None if item.tmdbType == 'movie' else peli['name'],
                contentType = 'movie' if item.tmdbType == 'movie' else 'tvshow',
                infoLabels = {'year': peli['release_date' if item.tmdbType == 'movie' else 'first_air_date'].split("-")[0]}))
...
def get_tmdb_recomendados(item):
    ...
    if isinstance(resultados, dict) and 'results' in resultados:
        for peli in resultados['results']:
            itemlist.append(Item(
                channel = item.channel,
                title = '[TMDB] '+peli['title' if item.tmdbType == 'movie' else 'name'],
                action = 'search_similares',
                viewmode='list',
                viewcontent = 'movies',
                contentTitle = peli['title' if item.tmdbType == 'movie' else 'name'],
